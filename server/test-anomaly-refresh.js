...
async function testAnomalyRefresh() {
  try {
    ...
    const loginResponse = await makeRequest('http://127.0.0.1:5000/api/auth/login', {
      method: 'POST',
    ...
    const treatResponse = await makeRequest(`http://127.0.0.1:5000/api/anomalies/${anomalie.id}/validate`, {
      method: 'PUT',
      headers: {
    ...
    const getResponse = await makeRequest(`http://127.0.0.1:5000/api/anomalies?date=${anomalie.pointage.date}`, {
