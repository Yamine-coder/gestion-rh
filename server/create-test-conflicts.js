// Script pour tester les confli      await prisma.conge.create({
        data: {
          userId: cuisinier.id,
          type: i === 0 ? 'CP' : (i === 1 ? 'RTT' : 'CP'),
          dateDebut: dateConflitCritique,
          dateFin: finConflitCritique,
          motif: `Vacances de fin d'année - ${cuisinier.nom}`,
          statut: statut,
          vu: true
        }
      });s
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await prisma.conge.create({
        data: {
          ...
          motif: `Congé de fin d'année - ${serveur.nom}`,
          statut: statut,
          vu: true,
          createdAt: new Date(),
          updatedAt: new Date()
