-- =====================================================
-- SYSTÈME DE SCORING EMPLOYÉS - Tables et données
-- =====================================================
...
CREATE TABLE IF NOT EXISTS employe_points (
  id SERIAL PRIMARY KEY,
  employe_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
-- Supprime les anciennes règles si re-exécution
DELETE FROM scoring_rules WHERE true;
...
-- Vérification
SELECT code, label, points, categorie, type FROM scoring_rules ORDER BY categorie, points DESC;
