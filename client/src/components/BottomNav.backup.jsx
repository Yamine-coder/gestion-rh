...
export default function BottomNav({ pendingLeaves = 0, hasNotifications = false }) {
  ...
  return (
    <>
      ...
      {/* Mobile: Bottom Navigation Bar - Normes iOS/Material Design */}
      <nav 
        className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/98 dark:bg-slate-900/98 backdrop-blur-xl border-t border-slate-200/60 dark:border-slate-800/60 shadow-[0_-2px_16px_rgba(0,0,0,0.06)] dark:shadow-[0_-2px_20px_rgba(0,0,0,0.3)]" 
        style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
        role="navigation" 
        aria-label="Navigation principale"
      >
        {/* Container avec hauteur fixe 56px (Material Design standard) */}
        <div className="h-14 px-2">
          <div className="mx-auto h-full max-w-md grid grid-cols-5 items-center">
            {items.map(item => item.cta ? (
              /* FAB Central - 56px (Material standard) */
              <button 
                key={item.key} 
                onClick={openQuickQR} 
                aria-label={item.label} 
                className="relative -mt-7 justify-self-center group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/50 rounded-full"
              >
                <div 
                  className="grid place-items-center h-14 w-14 rounded-full bg-gradient-to-br from-primary-500 via-primary-600 to-primary-700 text-white shadow-lg shadow-primary-600/30 ring-[3px] ring-white dark:ring-slate-900 transition-transform duration-200 ease-out group-active:scale-95"
                >
                  <item.icon className="h-7 w-7" strokeWidth={1.5} />
                </div>
              </button>
            ) : (
              /* Nav Items - Touch target 48x48px minimum */
              <NavLink 
                key={item.key} 
                to={item.to} 
                className={({ isActive }) => [
                  "relative flex flex-col items-center justify-center h-12 min-w-[48px] rounded-lg transition-colors duration-200",
                ...
                {({ isActive }) => (
                  <>
                    ...
                    <div className="relative">
                      ...
                      {/* Badge */}
                      {item.badge && (
                        <span className="absolute -top-1.5 -right-2 min-w-[18px] h-[18px] px-1 rounded-full bg-primary-600 text-white text-[10px] leading-[18px] text-center font-semibold shadow-sm">
                          {item.badge}
                        </span>
                      )}
                      ...
                      {/* Notification Dot */}
                      {item.dot && !item.badge && (
                        <span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full bg-primary-500 ring-[1.5px] ring-white dark:ring-slate-900" />
                      )}
                    </div>
                    
                    {/* Active indicator dot */}
                    <div className={`mt-1 h-1 w-1 rounded-full transition-all duration-200 ${isActive ? 'bg-primary-600 dark:bg-primary-400 scale-100' : 'scale-0'}`} />
                  </>
                )}
              </NavLink>
            ))}
