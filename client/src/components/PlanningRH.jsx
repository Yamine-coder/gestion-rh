...
import EmployeeTemplatePreview from './EmployeeTemplatePreview';
import CategoryPresenceModal from './CategoryPresenceModal';
...
function PlanningRHTable({ 
  ...
  // State pour la prévisualisation du planning type d'un employé
  const [templatePreviewEmp, setTemplatePreviewEmp] = useState(null);
  // State pour la modale "qui est présent à quelle heure" par catégorie
  const [presenceCategorie, setPresenceCategorie] = useState(null);
  // Menu kebab d'actions planning par employé (création/modif/suppression)
  const [rowMenuEmpId, setRowMenuEmpId] = useState(null);
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden min-w-0">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-52 sm:w-52 flex-shrink-0 flex flex-col h-full">
          ...
          {employesGroupesParCategorie.map((groupe, groupeIndex) => (
            <React.Fragment key={groupe.categorie}>
              ...
              <div className="group bg-gray-50 border-b border-r border-gray-200 px-3 py-2 flex items-center justify-between" style={{ height: '36px' }}>
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-1 h-4 bg-[#cf292c] rounded-full" />
                  <span className="text-[11px] font-bold text-gray-800 truncate">{groupe.categorie}</span>
                  <span className="text-[10px] text-gray-500 font-medium">({groupe.employes.length})</span>
                </div>
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); setPresenceCategorie(groupe); }}
                  className="flex-shrink-0 p-1 rounded-md text-gray-400 hover:text-[#cf292c] hover:bg-white transition-colors opacity-60 hover:opacity-100"
                  title="Voir qui est présent à quelle heure"
                >
                    ...
                    <div className={`flex items-center h-full px-2 gap-2 ${viewType === "mois" ? '' : 'pt-2 items-start'}`}>
                      ...
                      <div className="flex-1 min-w-0 flex items-center justify-between gap-1">
                        <div className="flex-1 min-w-0 flex items-center gap-1">
                          ...
                          {/* Icône planning type */}
                          <button
                            onClick={(e) => { e.stopPropagation(); setTemplatePreviewEmp(emp); }}
                            className="p-0.5 rounded hover:bg-gray-200/80 text-gray-400 hover:text-[#cf292c] opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
                            title="Voir planning type"
                          >
                            <LayoutGrid className="w-3 h-3" />
                          </button>
                          {/* Actions rapides planning (kebab) */}
                          {isAdmin && (
                            <div className="relative flex-shrink-0">
                              <button
                                ...
                                title="Actions planning"
                                aria-label={`Actions planning pour ${formatEmployeeName(emp)}`}
                              >
                                <MoreVertical className="w-3.5 h-3.5" />
                              </button>
                              {rowMenuEmpId === emp.id && (
                                <>
                                  <div className="fixed inset-0 z-40" onClick={(e) => { e.stopPropagation(); setRowMenuEmpId(null); }} />
                                  <div className="fixed z-50 w-44 bg-white rounded-lg shadow-lg border border-gray-200 py-1 overflow-hidden" style={{ top: rowMenuPos.top, left: rowMenuPos.left }} onClick={(e) => e.stopPropagation()}>
                                    <div className="px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wide truncate border-b border-gray-100">
                                      {formatEmployeeName(emp)}
                                    </div>
                                    <button
                                      type="button"
                                      onClick={() => { onPlanningAction('create', emp.id); setRowMenuEmpId(null); }}
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => { onPlanningAction('modify', emp.id); setRowMenuEmpId(null); }}
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => { onPlanningAction('delete', emp.id); setRowMenuEmpId(null); }}
                                      className="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors"
                                    >
        ...
        {/* Modal présence par catégorie */}
        {presenceCategorie && (
          <CategoryPresenceModal
            groupe={presenceCategorie}
            dates={dates}
            shifts={shifts}
            conges={conges}
            formatDate={formatDate}
            onClose={() => setPresenceCategorie(null)}
          />
        )}
      </div>
...
function DayAgenda({ date, employes, shifts, conges, onCellClick, formatEmployeeName, getEmployeeInitials, isAdmin = false, onPlanningAction = () => {} }) {
  const today = new Date();
  ...
  const [rowMenuEmpId, setRowMenuEmpId] = useState(null); // Menu kebab d'actions planning par employé
  const [rowMenuPos, setRowMenuPos] = useState({ top: 0, left: 0 });
          ...
          return (
            ...
            >
              ...
              <div className="w-40 shrink-0 border-r border-gray-200 px-2 py-1.5 flex items-center gap-2">
                ...
                {/* Actions rapides par employé (kebab) */}
                {isAdmin && (
...
export default function PlanningRH({ openAnomaliesPanel = false }) {
  ...
  // Pré-sélection pour l'assistant (ouverture contextuelle depuis une ligne employé)
  const [creationRapideInitialTab, setCreationRapideInitialTab] = useState('create');
  const [creationRapideInitialEmployees, setCreationRapideInitialEmployees] = useState([]);
  // Ouvre l'assistant pré-rempli pour un employé et un onglet donnés
  const openCreationRapide = (tab = 'create', employeeIds = []) => {
    setCreationRapideInitialTab(tab);
    setCreationRapideInitialEmployees(employeeIds);
    setCreationRapideModalOpen(true);
  };
  const [showTemplatesPanel, setShowTemplatesPanel] = useState(false);
  const [activeTemplateInfo, setActiveTemplateInfo] = useState(null); // {nom, categorie} from PlanningTypes
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setEmployes((employesRes.data || []).filter(e => e.statut === 'actif'));
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
        ...
        {showFloatingToolbar && !showTemplatesPanel && (
          <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40 animate-in slide-in-from-bottom-4 duration-300">
            <div className="bg-white/95 backdrop-blur-sm shadow-xl rounded-2xl border border-gray-200/70 px-4 py-3 flex items-center gap-3">
              ...
              {/* Bouton Templates - toggle vue inline */}
              <button
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <header className="fixed top-16 left-0 right-0 h-14 bg-white border-b border-gray-200 z-40">
            {showTemplatesPanel ? (
              <div className="h-full max-w-[1800px] mx-auto px-4 flex items-center gap-2.5">
                <button
                  onClick={() => setShowTemplatesPanel(false)}
                  className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition"
                ...
                <div className="w-7 h-7 rounded-lg bg-[#cf292c] flex items-center justify-center shadow-sm flex-shrink-0">
                  <LayoutGrid className="w-3.5 h-3.5 text-white" />
                </div>
                <span className="text-sm font-bold text-gray-900 whitespace-nowrap">Templates</span>
                {activeTemplateInfo && (
                  <>
                    <div className="w-px h-5 bg-gray-200" />
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-50 border border-red-200">
                      <span className="text-xs font-semibold text-[#cf292c] truncate max-w-[200px]">{activeTemplateInfo.nom}</span>
                      <span className="text-[10px] text-red-400 font-medium">{activeTemplateInfo.categorie}</span>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="h-full max-w-[1800px] mx-auto px-4 flex items-center gap-4">
              ...
              <button
                ...
                title="Gérer les templates de planning"
              >
                <LayoutGrid className="w-3 h-3" />
                <span>Templates</span>
              </button>
              ...
              <div className="hidden lg:flex items-center gap-1.5">
                ...
                {/* Bouton Nouveau */}
                <button
                  onClick={() => openCreationRapide('create', [])}
            ...
            {mobileMenuOpen && (
              <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 px-4 py-3 space-y-3 shadow-md">
                <div className="flex gap-2">
                  ...
                  {/* Bouton Templates mobile */}
                  <button
                    onClick={() => setShowTemplatesPanel(v => !v)}
                    className={`relative flex-1 px-2.5 py-2 text-xs font-medium rounded-lg transition flex items-center justify-center gap-1 ${showTemplatesPanel ? 'bg-[#cf292c] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50 border border-red-200 bg-red-50'}`}
                  >
                    <LayoutGrid className={`w-3.5 h-3.5 ${showTemplatesPanel ? 'text-white' : 'text-[#cf292c]'}`} />
                    <span className={showTemplatesPanel ? 'text-white' : 'text-red-700'}>Templates</span>
                  </button>
                </div>
                <div className="relative">
                  <input
          ...
          {/* Légende des types de segments - uniquement en vue semaine (masquée en mode Templates) */}
          {!showTemplatesPanel && viewType === 'semaine' && (
          ...
          {/* Légende mode comparaison - uniquement en vue semaine (masquée en mode Templates) */}
          {!showTemplatesPanel && showComparaison && viewType === 'semaine' && (
            ...
            <main className={`fixed ${
              ...
              {showTemplatesPanel ? (
                <div className="h-full bg-white">
                  <PlanningTypes embedded={true} employesProp={employes} onClose={() => setShowTemplatesPanel(false)} onTemplateSelect={(t) => setActiveTemplateInfo(t ? { nom: t.nom, categorie: t.categorie } : null)} />
                </div>
              ) : filteredEmployes.length === 0 ? (
                ...
                <>
                  ...
                  {viewType === 'jour' ? (
                    <DayAgenda
                      ...
                      isAdmin={isAdmin}
                      onPlanningAction={(tab, empId) => openCreationRapide(tab, [empId])}
                    />
                  ) : viewType === 'mois' ? (
                    <MonthCalendarView
                      dates={dates}
        ...
        {modalOpen && selected && (
            ...
            {creationRapideModalOpen && (
              <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-150">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] sm:max-h-[85vh] overflow-hidden border border-slate-200 flex flex-col">
                  {/* Header avec badge icône */}
                  <div className="flex-shrink-0 px-5 py-3.5 border-b border-slate-100 bg-gradient-to-r from-white to-slate-50">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-[#cf292c] text-white flex items-center justify-center shadow-sm shadow-red-200">
                          <Calendar className="w-5 h-5" strokeWidth={1.8} />
                        </div>
                        <div>
                          <h2 className="text-[15px] font-semibold text-slate-800 leading-tight">Gestion rapide des plannings</h2>
                          <p className="text-[11px] text-slate-400 leading-tight">Créer, modifier ou supprimer en quelques clics</p>
                        </div>
                      </div>
                      <button 
                  ...
                  <div className="flex-1 overflow-y-auto p-5">
                    ...
                    <CreationRapideForm 
                      key={`crf-${creationRapideInitialTab}-${creationRapideInitialEmployees.join('_')}`}
                      employes={employes}
                      initialTab={creationRapideInitialTab}
                      initialEmployeeIds={creationRapideInitialEmployees}
      ...
      {/* Modale Extras - Style cohérent avec Remplacements */}
      {showExtrasPanel && (
        <div 
